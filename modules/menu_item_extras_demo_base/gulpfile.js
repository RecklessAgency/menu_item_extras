 ...
 ************************/
gulp
  .task('js-lint', () => gulp.src(configs.allScripts)
      .pipe(jshint())
      ...
      .pipe(notify((file) => {
        ...
        const errors = file.jshint.results.map(data => (data.error ? `(${data.error.line}: ${data.error.character}) ${data.error.reason}` : false)).join('\n');
...
gulp
  .task('js-standalone', (done) => {
    glob(configs.standaloneScripts, {}, (err, files) => {
      if (err) done(err);
      files.map((entry) => {
        webpackStandaloneConfig.entry[path.basename(entry)] = entry;
        return entry;
      });
